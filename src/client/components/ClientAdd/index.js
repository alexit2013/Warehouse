...
class ClientAdd extends React.Component{
    ...
    handlerAdd( ){
        if( this.state.phone.length>0 && this.state.name.length>0)
            this.props.clientAdd(this.state);
    }
    ...
    render(){
        return(
            <WrapperApp  >
                <Header/>
                <Wrapper>
                    <TitleWrapper>Добавление клиента</TitleWrapper>
                    ...
                    <Button onClick={this.handlerAdd} color="success">Добавить</Button>
                </Wrapper>
            </WrapperApp>
        )
    }
